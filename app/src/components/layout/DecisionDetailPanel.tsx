...
export default function DecisionDetailPanel({ onAddEdge }: DecisionDetailPanelProps) {
  ...
  return (
    <aside className="w-80 bg-gray-900 border-l border-gray-700 h-full flex flex-col overflow-hidden shadow-lg">
      {/* Header */}
          ...
          <div className="flex-1 min-w-0">
            ...
            {isEditing ? (
              <input
                ...
                className="w-full font-semibold text-gray-100 text-lg px-2 py-1 bg-gray-800 border border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            ) : (
        ...
        {isEditing && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-700">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-gray-700 text-gray-200 text-sm rounded hover:bg-gray-600"
            >
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {node.type === 'decision' && (
          <div className="mb-4 p-3 bg-amber-950/50 rounded-lg border border-amber-800">
            <div className="text-xs font-medium text-amber-300 mb-1">Ja/Nee Vraag</div>
            {isEditing ? (
              <textarea
                value={editedNode?.vraag || ''}
                onChange={(e) => updateField('vraag', e.target.value)}
                className="w-full px-2 py-1 text-sm bg-gray-800 border border-gray-600 text-gray-100 rounded focus:outline-none focus:ring-2 focus:ring-amber-500 resize-y min-h-[60px]"
                placeholder="De vraag die met Ja of Nee beantwoord wordt..."
              />
            ) : (
              <p className="text-sm text-amber-300">{node.vraag || <span className="text-amber-700 italic">Geen vraag ingesteld</span>}</p>
            )}
          </div>
        )}
        ...
        {/* Beschrijving */}
        <div className="mb-4">
          <div className="text-xs font-medium text-gray-500 mb-1">Beschrijving</div>
          {isEditing ? (
            <textarea
              value={editedNode?.beschrijving || ''}
              onChange={(e) => updateField('beschrijving', e.target.value)}
              className="w-full px-2 py-1 text-sm bg-gray-800 border border-gray-600 text-gray-100 rounded focus:outline-none focus:ring-2 focus:ring-purple-500 resize-y min-h-[60px]"
              placeholder="Optionele beschrijving..."
            />
          ) : (
            <p className="text-sm text-gray-300">{node.beschrijving || <span className="text-gray-500 italic">Geen beschrijving</span>}</p>
          )}
        </div>
        ...
        {(node.type === 'action' || node.type === 'subprocess') && (
          <div className="mb-4">
            <div className="text-xs font-medium text-gray-500 mb-1">Verantwoordelijke afdeling</div>
            {isEditing ? (
              <select
                value={editedNode?.afdeling || ''}
                onChange={(e) => updateField('afdeling', e.target.value)}
                className="w-full px-2 py-1.5 text-sm bg-gray-800 border border-gray-600 text-gray-100 rounded focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
              ...
              <p className="text-sm">
                {node.afdeling ? (
                  ...
                  <span className="text-gray-500 italic">Geen afdeling</span>
                )}
              </p>
            )}
          </div>
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-semibold text-gray-300 text-sm">Verbindingen</h3>
          </div>
            ...
            <div className="mb-3">
              ...
              <div className="space-y-1">
                {incomingEdges.map((edge) => {
                  ...
                  return (
                    <div key={edge.id} className="text-xs p-2 bg-gray-800 rounded flex items-center justify-between group">
                      <div className="flex items-center gap-2">
                        {sourceNode && NODE_TYPE_ICONS[sourceNode.type]}
                        <span className="text-gray-300">{sourceNode?.titel || edge.van}</span>
                        {edge.label && (
                  ...
                  return (
                    <div key={edge.id} className="text-xs p-2 bg-gray-800 rounded flex items-center justify-between group">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300">{targetNode?.titel || edge.naar}</span>
                        {edge.label && (
                          <Badge variant={edge.type === 'ja' ? 'green' : edge.type === 'nee' ? 'red' : 'default'}>
                            {edge.label}
                          </Badge>
          ...
          {incomingEdges.length === 0 && outgoingEdges.length === 0 && (
            <p className="text-xs text-gray-500 italic mb-2">Geen verbindingen</p>
          )}

          <button
            onClick={() => onAddEdge?.()}
            className="flex items-center gap-1 text-xs text-purple-400 hover:text-purple-300 px-2 py-1 hover:bg-gray-700 rounded"
          >
        ...
        {node.type === 'subprocess' && (node.linkedProcessId || node.linkedFlowchartId) && (
          <div className="mb-4 p-3 bg-purple-950/50 rounded-lg border border-purple-800">
            <div className="text-xs font-medium text-purple-300 mb-1">Koppeling</div>
            {node.linkedProcessId && (
              <p className="text-sm text-purple-300">Gekoppeld aan proces: {node.linkedProcessId}</p>
            )}
            {node.linkedFlowchartId && (
              <p className="text-sm text-purple-300">Gekoppeld aan flowchart: {node.linkedFlowchartId}</p>
            )}
          </div>
        )}
      </div>
