...
import { ChevronLeft, ChevronRight, ChevronDown, Filter, X, Search, FileText } from 'lucide-react';
  ...
  return (
    <div className="mb-2 border-b border-gray-700 pb-2">
      <button
        ...
        className="w-full flex items-center justify-between py-2 px-1 hover:bg-gray-700 rounded transition-colors"
      >
        ...
        {selectedCount > 0 && (
          <span className="text-[10px] bg-blue-900/60 text-blue-300 px-1.5 py-0.5 rounded-full font-medium">
            {selectedCount}
          </span>
        )}
      </button>
  ...
  return (
    <div className="space-y-0.5">
      {sortedOpties.map((optie) => (
        <label
          key={optie.id}
          className="flex items-center gap-2 text-sm text-gray-400 cursor-pointer hover:bg-gray-700 px-2 py-1.5 rounded"
        >
          <input
            type="checkbox"
            checked={selected.includes(optie.id)}
            onChange={() => handleToggle(optie.id)}
            className="rounded border-gray-600 text-blue-600 focus:ring-blue-500 w-3.5 h-3.5"
          />
  ...
  return (
    <div className="space-y-1">
      {sortedOpties.map((optie) => (
        <label
          key={optie.id}
          className="flex items-start gap-2 text-sm text-gray-400 cursor-pointer hover:bg-gray-700 px-2 py-2 rounded"
          style={{
        ...
        >
          <input
            ...
            checked={selected.includes(optie.id)}
            onChange={() => handleToggle(optie.id)}
            className="rounded border-gray-600 text-blue-600 focus:ring-blue-500 w-3.5 h-3.5 mt-0.5"
          />
          <div className="flex-1">
            <div className="font-medium text-[13px]">{optie.label}</div>
            {optie.beschrijving && (
              <div className="text-[11px] text-gray-500">{optie.beschrijving}</div>
            )}
          </div>
        </label>
      ))}
...
export default function Sidebar() {
  ...
  const bronTekst = useStore((state) => state.bronTekst);
  ...
  if (!isSidebarOpen) {
    return (
      <button
        ...
        className="absolute left-4 top-20 z-10 bg-gray-800 p-2 rounded-lg shadow-md border border-gray-700 hover:bg-gray-700 transition-colors"
  ...
  return (
    <aside className="w-64 bg-gray-900 border-r border-gray-700 h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        <div className="mb-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-sm bg-gray-800 border border-gray-600 text-gray-100 placeholder-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            {filters.zoekterm && (
              <button
        ...
        {bronTekst !== null && (
          <div className="mb-2 border-b border-gray-700 pb-2">
            <button
              onClick={() => {
                const el = document.getElementById('brontekst-section');
                if (el) el.classList.toggle('hidden');
              }}
              className="w-full flex items-center gap-2 py-2 px-1 hover:bg-gray-700 rounded transition-colors"
            >
              <FileText className="w-4 h-4 text-gray-400" />
              <span className="text-xs font-semibold text-gray-300 uppercase tracking-wide">
                Brondocument
              </span>
              <span className="ml-auto text-[10px] bg-green-900/60 text-green-300 px-1.5 py-0.5 rounded-full">
                opgeslagen
              </span>
            </button>
            <div id="brontekst-section" className="hidden pt-1">
              <textarea
                value={bronTekst}
                onChange={(e) => setBronTekst(e.target.value)}
                className="w-full h-48 p-2 text-xs font-mono bg-gray-800 border border-gray-600 text-gray-300 placeholder-gray-500 rounded-lg resize-y focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Brondocument tekst..."
              />
              <p className="text-[10px] text-gray-500 mt-1 px-1">
                Wijzigingen worden meegeslagen bij het opslaan van het canvas.
              </p>
            </div>
          </div>
        )}
